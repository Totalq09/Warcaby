...
//******************************************************
	...
	if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
	{
		...
		{
			if ((isPlayerTurn == true && (board.getElementStatus(mouse.pos) == Status::Player
                || board.getElementStatus(mouse.pos) == Status::PlayerKing))
                || (isPlayerTurn == false && (board.getElementStatus(mouse.pos) == Status::Enemy
                || board.getElementStatus(mouse.pos) == Status::EnemyKing)))
			{
                //jeśli jest bicie i kliknęliśmy na niewłaściwy pionek -> zignoruj kliknięcie
                if(path.size() > 1 && (mouse.pos.x != path[0].x || mouse.pos.y != path[0].y))
                    return false;
		...
		else if (currentState == EngineState::PawnPicked)
		{
			...
			{
                //jeśli jest bicie i nie chcemy bić -> zignoruj kliknięcie
                if(path.size() > 1 && (mouse.pos.x != path[1].x || mouse.pos.y != path[1].y))
                    return false;

				if (board.movePawn(board.getSelectedElementPosition(),
                        mouse.pos, board.getSelectedElementStatus()) == false)
					return false;

				currentState = EngineState::None;

				//w razie gdyby było podwójne bicie
				if(path.size() > 2)
                    return false;
				return true;
			}
